...
// ==================== 阶段四：密钥协商（LWE密钥交换）====================
// 根据require.txt第25-31行和第58-64行的要求

// 生成小误差向量（用于LWE）
inline vec_ZZ_p generate_error_vector(int n, u64 error_bound = 3){
    vec_ZZ_p e;
    e.SetLength(n);
    for(int i = 0; i < n; i++){
        // 生成小的随机误差 [-error_bound, error_bound]
        long error = (RandomBnd(2 * error_bound + 1) - error_bound);
        e[i] = conv<ZZ_p>(error);
    }
    return e;
}

// User端：密钥协商第1步 - 生成公钥向量 a = Hash(rw)
inline vec_ZZ_p generate_public_vector_a(u64 rw, int n_vector){
    // 将rw转换为字符串并hash
    std::string rw_str = std::to_string(rw);
    return hash_to_vecZZp(rw_str, n_vector);
}

// 生成秘密向量 s = Hash(session)
inline vec_ZZ_p generate_secret_vector_s(const std::string &session, int n_vector){
    return hash_to_vecZZp(session, n_vector);
}

// User端：密钥协商第2-3步 - 计算 b2 = a*s2 + e1
inline vec_ZZ_p compute_b2(const vec_ZZ_p &a, const vec_ZZ_p &s2, const vec_ZZ_p &e1){
    vec_ZZ_p b2;
    b2.SetLength(a.length());
    // b2 = a * s2 + e1（逐分量计算）
...
// Server端：密钥协商第1-2步 - 计算 b1 = a*s1 + e2
inline vec_ZZ_p compute_b1(const vec_ZZ_p &a, const vec_ZZ_p &s1, const vec_ZZ_p &e2){
    vec_ZZ_p b1;
...
// User端：密钥协商第6步 - 计算协商密钥 k = <b1, s2>
inline ZZ_p derive_shared_key_user(const vec_ZZ_p &b1, const vec_ZZ_p &s2){
    ZZ_p k;
    InnerProduct(k, b1, s2);
    return k;
}

// Server端：计算协商密钥 k = <b2, s1>
inline ZZ_p derive_shared_key_server(const vec_ZZ_p &b2, const vec_ZZ_p &s1){
    ZZ_p k;
    InnerProduct(k, b2, s1);
    return k;
}

// 从协商密钥导出最终的会话密钥（提取低位比特以减少误差影响）
inline u64 extract_session_key(const ZZ_p &k, int bit_length = 16){
    u64 k_u64 = conv<unsigned long>(k);
    // 提取低bit_length位作为会话密钥
    u64 mask = (1ULL << bit_length) - 1;
    return k_u64 & mask;
} 
